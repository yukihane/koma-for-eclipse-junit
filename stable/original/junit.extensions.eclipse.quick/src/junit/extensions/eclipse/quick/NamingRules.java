...
public class NamingRules {
    ...
    public NamingRules(final IPreferenceStore store) {
        this.store = store;
    }

    public List get() {
        final String value = store.getString(STORE_ID);
    ...
    public String[] getEnableValues() {
        final List namingRules = get();
        final List result = new ArrayList();
        for (int i = 0; i < namingRules.size(); ++i) {
            final NamingRule rule = (NamingRule) namingRules.get(i);
            if (rule.isEnabled()) {
    ...
    public void set(final List namingRules) {
        store.setValue(STORE_ID, listToString(namingRules));
    }

    private String listToString(final List namingRules) {
    ...
    private List stringToList(final String string) {
        final List result = new ArrayList();
        ...
        while (st.hasMoreTokens()) {
            ...
            if (index != -1) {
                final String value = column.substring(0, index);
                final Boolean enabled = Boolean.valueOf(column.substring(index + 1));
